...
from app.data_models.order_tracking import OrderTrackingRequest, OrderResponse
...
def get_order_history(
    ...
    order = (
        ...
        .filter(
            ...
            User.zem_name == current_user,
        )
        .first()
    )

    if not order:
        raise HTTPException(status_code=404, detail=f"No matching orders found for {container_number}")
